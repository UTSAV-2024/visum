...
- [x] Verify the deployed site scores 90+ on its own scanner — **confirmed: production scores 100/100** ("Excellent — AI Optimized") at https://visum-eight.vercel.app. Every check maxed: robots 15/15, JSON-LD 20/20, llms.txt 10/10, MCP 10/10, JS-rendering 10/10, meta 10/10, sitemap 5/5, Page Load Speed 10/10 (real TTFB 117ms, load 806ms). Note: the earlier 89/100 was **not** a localhost/CDN artifact — it was a genuine measurement bug (the crawler charged its own 2s hydration wait to the site, making a 10/10 speed score mathematically unreachable for *every* scanned site). Fixed via the Navigation Timing API.
